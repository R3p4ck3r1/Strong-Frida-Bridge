...
>- Create frida folder inside the target app's private files directory (`<getFilesDir()>/secenv`).  
>  Real path syntax: `/data/data/<container_pkg>/<rootfs>/data/user/<user_id>/<target_pkg>/files/secenv`.  
...
All files live under `<getFilesDir()>/secenv/`:

| File | Purpose |
|---|---|
| `libsecure-env.so` | Frida Gadget shared library (you supply this) |
| `libsecure-env.config.so` | Gadget config JSON — auto-managed by the bridge |
| `libsecure-env.script.so` | Compiled JS agent (you supply this, for script mode) |
| `secenv-bridge.cfg` | Bridge config — currently supports `delay=<seconds>` |

**Automatic path resolution**  
Whatever you put in the config’s `"path"` field (absolute, relative, real, or logical), the bridge will fix it at runtime. It copies the script file into the real gadget directory and rewrites the `"path"` to the bare filename `libsecure-env.script.so`. This guarantees the gadget can always find the script, even when the filesystem is remapped by a container.

---

## ⏱️ 3. Delay Configuration

Create `secenv-bridge.cfg` in the same `secenv/` directory:
...
Add `lib/<abi>/libsecenv-bridge.so` for each architecture, and place gadget
files under `files/secenv/` as described in section 2.
...
public class MyApp extends Application {
    static {
        System.loadLibrary("secenv-bridge");
    }
}
```
...
- **Script compilation**: If your script requires compilation, use `frida-compile` and ensure the output file is named `libsecure-env.script.so`.
...
- **Timing / hooks missing**: If some hooks don’t trigger, increase the `delay` value in `secenv-bridge.cfg`. This gives the app more time to reach the target code before the script runs.
